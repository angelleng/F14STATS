...
while (1) {
  ...
  label_old <- label
  obj = c(obj, sum(sapply(1:3, 
                          function(i) sum(apply(mix[label == i, ], 1, 
                                                function(y) sqrt(sum((centroids[i,] - y)^2)))))))
...
library(mvtnorm)
tao = matrix(0, 300, 3)
for (l in 1: 30) { # converge criterion to be added, for now just using number of iteration, works well 
  for (n in 1:300) {
